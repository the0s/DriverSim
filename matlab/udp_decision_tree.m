      ...
      if mod(p,400) == 0
           ...
           if size(User_Cell{1},1) ~= 0 
               for l=1:size(User_Cell,2)
                   ...
                   prediction_good(f)= decisionTree(angle_var,'good',instance);
                   prediction_ugly(s)= decisionTree(angle_var,'ugly',instance);
                   s=s+1;
                   f=f+1;
               end          
           end
           if size(User_Cell_Bad{1},1) ~= 0 
               for l=1:size(User_Cell_Bad,2)
                   ...
                   prediction_bad(f)= decisionTree(angle_var,'bad',instance); 
                   g=g+1;
                   prediction_ugly(s)= decisionTree(angle_var,'ugly',instance);
                   s=s+1;
               end          
